...
const PairingProtocol = require('./pairing-protocol');
...
class HttpClient {
   ...
   * @return {Promise} Promise which resolves to the generated session keys.
   */
   ...
   * @return {Promise} Promise which resolves when the process is complete.
   */
   ...
   *
   * @return {Promise} Promise which resolves to the final TLV when the process
   *                   is complete.
   */
  listPairings() {
    const connection = new HttpConnection(this.address, this.port);
   ...
   * @param {boolean} meta - Whether or not to retrieve metadata
   * @param {boolean} perms - Whether or not to retrieve permission data
   * @param {boolean} type - Whether or not to retrieve the characteristic type
   * @param {boolean} ev - Whether or not to retrieve the 'ev' property
   * @returns {Promise} Promise which resolves to the JSON document.
   */
  getCharacteristics(characteristics, meta = false, perms = false, type = false,
                     ev = false) {
    const connection = new HttpConnection(this.address, this.port);
    return this._pairVerify(connection).then((keys) => {
      connection.setSessionKeys(keys);
   ...
   *
   * @param {Object} characteristics - Characteristics ID to set, id -> val
   * @returns {Promise} Promise which resolves to the JSON document.
   */
