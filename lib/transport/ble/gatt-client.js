...
class GattClient extends EventEmitter {
  ...
  pairSetup(pin) {
    ...
    return connection.connect().then(() => {
      ...
      if (status !== 0) {
        return Promise.reject(`Got error status while pairing: ${status}`);
      }
            ...
            return this.pairingProtocol.parsePairVerifyM2(
              ...
              if (status !== 0) {
                return Promise.reject(
                  `Got error status while pairing: ${status}`
                );
              }
   ...
   * @returns {Promise} Promise which resolves to the JSON document.
   */
  getCharacteristics(characteristics, options = {}, connection) {
    ...
    return promise.then(() => {
      ...
      for (const c of cList) {
        ...
        promises.push(connection.writeCharacteristic(c, [pdu]).then((pdus) => {
                ...
                if (Buffer.isBuffer(data) && data.length === 0) {
                  ...
                  connection.writeCharacteristic(
                    ...
                    for (const pdu of pdus) {
                      const tlv = TLV.decodeBuffer(
                        pdu.slice(5, pdu.length)
                      );

                      const value = tlv[GattConstants.Types['HAP-Param-Value']];
                          ...
                          characteristics: [
                            {
                              aid: 1,
                              iid: c.iid,
                              value: GattUtils.bufferToValue(value, c.format),
                            },
                          ],
                        }
                      );
