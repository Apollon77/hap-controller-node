    ...
    while (this._pending.length > 0) {
      switch (this._state) {
        ...
        case State.HEADERS_COMPLETE: {
          if(this.headers['content-length'] !== void 0) {
          ...
          } else if(this.headers['content-type'] !== void 0) {
            if(this._pending.indexOf(EVENT_SUFFIX, this._pending.length - EVENT_SUFFIX.length) !== -1)
            {
              let body = this._pending + ''
              const firstPosition = body.indexOf('{')
              const lastPosition = body.lastIndexOf('}')
              body = body.substr(firstPosition, lastPosition - firstPosition + 1)
